...
namespace ParkCars
{
    public partial class SystemView : Form
    {
        ...
        public SystemView(string name, string id, string username , string email, string address, string password)
        {
            InitializeComponent();
            Scan();
            timer2.Start();
        ...
        private void VehicleListButton_Click(object sender, EventArgs e)
        {
            ...
            if (!flag)
            {
                ...
                MessageBox.Show(w,"No Car Available in the parking slots" + MessageBoxIcon.Error);
            }
           ...
           // MessageBox.Show("Welcome");
            ...
            if (empty)
            {
                ...
                Task.Delay(TimeSpan.FromSeconds(2))
                    .ContinueWith((t) => w.Close(), TaskScheduler.FromCurrentSynchronizationContext());
                MessageBox.Show(w,"No such type User is available" + MessageBoxIcon.Error);
            }
            else
            {
                try
